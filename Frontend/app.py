...
@app.route('/Cabanas/<int:cabana_id>', methods=["GET"])#cabaña dinamica por id
def Cabana_especifica(cabana_id):

    response = requests.get(f'http://127.0.0.1:5050/posadas?identificador={cabana_id}')
    data = response.json()['posadas'][0]

    return render_template('Cabana_Amapola.html', cabana=data)#cambiar vista a otra donde se pueda ver la cabana en especifico
